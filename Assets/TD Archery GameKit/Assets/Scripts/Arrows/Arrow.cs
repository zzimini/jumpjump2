...
public class Arrow : MonoBehaviour {
	...
	void Update () {
		...
		if(transform.parent && transform.parent.name == "legs collider")
			transform.position = new Vector3(transform.position.x, transform.root.gameObject.GetComponent<Enemy>().leg.position.y + startDistance, transform.position.z);
	}
	...
	void OnTriggerEnter(Collider col){
		...
		if(!col.gameObject.GetComponent<Arrow>() && flying && !col.gameObject.GetComponent<FreezeArea>()){
			...
			if(enemyscript && !enemyscript.freeze){
				...
				if(Physics.Raycast(transform.position, fwd, out hit)){
					if(hit.distance < 1.5f && hit.transform.root.gameObject.GetComponent<Enemy>()){
						...
						GameObject bloodEffect = Instantiate(hit.transform.root.gameObject.GetComponent<Enemy>().bloodEffect, hit.point, transform.rotation) as GameObject;
			...
			if(legColliderNames.Contains(col.gameObject.name)){
				...
				startDistance = Mathf.Abs(transform.root.gameObject.GetComponent<Enemy>().leg.position.y - transform.position.y);
